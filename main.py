...
def main(args):
    if args.dataset =='SBHAR':
        users_T = np.arange(15, args.N_users_S, 1) 
        N_users = len(users_T)  
    ...
    acc, f1 = [np.empty([N_users], dtype=float) for _ in range(2)]
    ...
    print('\n=== ' + args.dataset + '_CASWL-Adapt ===')
    print("FINAL VALUE: \nacc: ", np.around(acc, 3), "\nf1: ", np.around(f1, 3))
    print("FINAL AVERAGE: \nacc: ", np.around(np.mean(acc), 3), "\nf1: ", np.around(np.mean(f1), 3))

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='PyTorch Implementation for CASWL-Adapt')
